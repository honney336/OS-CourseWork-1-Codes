...
double measure_hardware_swap() {
    ...
    // Windows Memory Usage Approximation (VmRSS equivalent)
    double mem_usage = 10.0;
    MEMORYSTATUSEX statex;
    statex.dwLength = sizeof(statex);
    if (GlobalMemoryStatusEx(&statex)) {
        // Scaling total physical memory as a proxy for environment load
...
int main() {
    ...
    printf("CampusConnect SJF Scheduler (Windows)\n");

    // STATIC DATA FROM YOUR LINUX OUTPUT
    int static_pids[] = {1, 8, 4, 2, 5, 9, 3, 10, 7, 6};
    double static_at[] = {1.0, 4.0, 3.0, 3.0, 4.0, 5.0, 2.0, 1.0, 5.0, 3.0};
    double static_bt[] = {4.0, 2.0, 3.0, 5.0, 5.0, 5.0, 6.0, 6.0, 7.0, 8.0};
    ...
    for (int i = 0; i < n; i++) {
        ...
        // SJF Logic: Find shortest burst time among arrived processes
        for (int j = 0; j < n; j++) {
